import type { Env, Business, IngestRecord, BusinessStatus } from './types';
...
export async function listBusinesses(env: Env, params: ListParams): Promise<ListResult> {
  const { clause, bindings } = buildWhereClause(params);

  const countRow = await env.DB.prepare(`SELECT COUNT(*) as count FROM businesses ${clause}`)
    .bind(...bindings)
    .first<{ count: number }>();
  ...
  const { results } = await env.DB.prepare(
    `SELECT * FROM businesses ${clause} ORDER BY discovered_at DESC LIMIT ? OFFSET ?`
  )
...
export async function listAllForExport(
  ...
  const { clause, bindings } = buildWhereClause(params);
  const { results } = await env.DB.prepare(
    `SELECT * FROM businesses ${clause} ORDER BY discovered_at DESC`
  )
    .bind(...bindings)
    .all<Business>();
  return results ?? [];
...
export async function updateStatus(env: Env, placeId: string, status: BusinessStatus): Promise<boolean> {
  const result = await env.DB.prepare('UPDATE businesses SET status = ? WHERE place_id = ?')
    .bind(status, placeId)
    .run();
...
export async function getAllPlaceIds(env: Env): Promise<string[]> {
  const { results } = await env.DB.prepare('SELECT place_id FROM businesses').all<{ place_id: string }>();
